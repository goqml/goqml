...
func (model *QAbstractTableModel) Parent(child *QModelIndex) *QModelIndex {
	index := DosQAbstractTableModelParent(DosQAbstractTableModel(model.vptr), child.vptr)
...
func (model *QAbstractTableModel) Index(row int, column int, parent *QModelIndex) *QModelIndex {
	index := DosQAbstractTableModelIndex(DosQAbstractTableModel(model.vptr), row, column, parent.vptr)
	return NewQModelIndexFromOther(index, OwnershipTake)
}
...
func (model *QAbstractTableModel) Setup() {
	...
	model.vptr = DosQObject(DosQAbstractTableModelCreate(unsafe.Pointer(model), model.StaticMetaObject().vptr, qObjectCallback, qAIMCallbacks))
}
